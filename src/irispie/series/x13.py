    ...
    def x13(
        ...
        **kwargs,
        ...
        if not all(i["success"] for i in info):
            _wd.throw(when_error, "X13 failed to produce a result for at least one column.", )
        output_series = type(self).from_start_date_and_data(base_start_date, _np.hstack(output_data), dtype=float, )
...
def _remove_leading_trailing_nans(
    ...
    column_data: _np.ndarray,
    /,
) -> tuple[_da.Date, _np.ndarray, slice]:
    """
    """
    #[
