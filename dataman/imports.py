...
def _block_iterator(name_line, descript_row, data_lines, start_date_only, /, ):
    ...
    _is_start = lambda cell: cell.startswith("__") and da_.frequency_from_string(cell) is not da_.Frequency.UNKNOWN
    name_line += ["__"]
    status = False
    blocks = []
    ...
    for column, cell in enumerate(name_line):
        ...
        if not status and _is_start(cell):
            ...
            current_frequency = da_.frequency_from_string(cell)
    #]


def _extract_dates_from_data_lines(data_lines, frequency, column, start_date_only, /, ):
    """
    """
    #[
    start_date = da_.dater_from_sdmx_string(frequency, data_lines[0][column])
    date_extractor = {
        True: lambda i, line: start_date + i,
        False: lambda i, line: da_.dater_from_sdmx_string(frequency, line[column]),
    }[start_date_only]
    row_index_and_dates = [ 
