...
def _hpf_data(
    ...
    *,
    ...
    encompassing_span, new_start_date, new_end_date = _dates.get_encompassing_span(self, level, change, span, )
    span = [ t for t in encompassing_span ]
    data = self.get_data_from_to((new_start_date, new_end_date, ), )
    num_periods, num_variants = data.shape
    level_data, level_where = _prepare_constraints(level, span, )
    change_data, change_where = _prepare_constraints(change, span, )
...
def _prepare_constraints(
    ...
    span: list[_dates.Dater],
    /,
) -> tuple[_np.ndarray | None, list[int] | None, ]:
    #[
    if constraint is None:
        return None, None
    data = constraint.get_data(span, 0)
