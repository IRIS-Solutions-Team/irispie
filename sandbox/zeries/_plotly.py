...
class Mixin:
    ...
    def plot(
        ...
        *,
        ...
        minor_dtick_months = min(12//range[0].frequency, 1, )
        minor_dtick_string = f"M{12//range[0].frequency}"

        traces = (traces, ) if isinstance(traces, dict) else traces
        color_cycle = _it.cycle(_COLOR_ORDER)
        traces_cycle = _it.cycle(traces or ({}, ))

        for i, c, ts in zip(builtin_range(num_variants, ), color_cycle, traces_cycle, ):
            traces_settings = {
                "x": date_str,
                "y": data[:, i],
