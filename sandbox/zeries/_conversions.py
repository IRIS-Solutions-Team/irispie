...
def _aggregate_daily_to_regular(
    ...
    new_data = tuple(
        tuple(
            ...
            for variant in all_self_data.T
        )
...
def _aggregate_regular_to_regular(
    ...
    new_data_transposed = tuple(
        tuple(
            ...
            for within_data in self_data_variant.reshape((-1, factor))
        )
        for self_data_variant in self_data.T
    )
    #
...
def _disaggregate_flat(
    ...
    #
    new_data = _np.full((new_num_periods, self.num_variants), _np.nan, dtype=self.data_type)
...
def convert_pct(
    ...
    """
    """
    return 100*(convert_roc(1 + pct/100, from_freq, to_freq) - 1)
